"""ADK FunctionTools for sector analysis."""

import asyncio

from db.database import async_session
from db.crud import get_sector_rankings as _get_rankings
from db.models import SectorRanking, ComputedMetric, Company

from sqlalchemy import select


def get_sector_rankings(sort_by: str = "composite", top_n: int = 5) -> dict:
    ...
    async def _query():
        async with async_session() as db:
            rankings = await _get_rankings(db, sort_by=sort_col, limit=min(top_n, 11))
            return [
                {
                    "sector": r.sector,
