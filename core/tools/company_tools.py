"""ADK FunctionTools for company analysis. Async — safe for FastAPI routes
...
async def get_best_companies(
    ...
    return {
        "companies": companies,
        "count": len(companies),
        "filters": {"sector": sector, "min_esg": min_esg, "min_sharpe": min_sharpe},
    }


async def get_company_detail(symbol: str) -> dict:
...
async def compare_companies(symbols: list[str]) -> dict:
    ...
    results = []
    for sym in symbols:
        detail = await get_company_detail(sym)
